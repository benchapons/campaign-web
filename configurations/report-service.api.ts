...
const headers = {
  ...
  'X-Apig-AppCode': process.env.REPORT_SERVICE_API_KEY,
};

const ReportServiceClient = axios.create({
  baseURL: process.env.REPORT_SERVICE_API_URL,
  headers,
  timeout: timeout,
...
ReportServiceClient.interceptors.request.use(
  (config: any) => {
    const uuid = uuidv4();
...
ReportServiceClient.interceptors.response.use(
  (response: any) => {
    try {
      const arraySender = [
