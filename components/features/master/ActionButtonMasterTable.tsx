...
import { MdEditNote, MdOutlineBlock } from 'react-icons/md';
    ...
    <div className="flex gap-2.5 text-xl">
      {isPermissionEdit && (
        <button
          ...
          onClick={onClickEdit}
        >
          <FiEdit3 className={isDisableEdit ? 'text-gray-dark' : 'text-blue-pacific'} />
        </button>
      )}
      {isPermissionDelete && (
        <button
          ...
          className="disabled:text-gray-bluish disabled:cursor-not-allowed"
          test-id={`reward-inactive-${index}`}
          onClick={onClickDelete}
        >
          <MdOutlineBlock className={isDisableDelete ? 'text-gray-dark' : 'text-red'} />
        </button>
      )}
      {isPermissionAuditLog && (
        <button
...
export default ActionButtonMasterTable;
